...
const Rocket = forwardRef<AnimatedIconHandle, AnimatedIconProps>(
    (
        ...
        // Idle / Rumble animation (for Loading & Hover)
        ...
        // Full Launch Sequence (for Click / Play)
        ...
        const stop = useCallback(() => {
            animate(
                ".rocket-upper, .rocket-flame, .rocket-body",
                { x: 0, y: 0, opacity: 1, scale: 1 },
                { duration: 0.2 },
            );
        }, [animate]);
        ...
        return (
            <motion.svg
                ref={scope}
                onMouseEnter={() => !loading && rumble()} // Differentiated: Hover = Rumble
                onMouseLeave={() => !loading && stop()}
                onClick={() => !loading && start()}       // Differentiated: Click = Launch
            ...
            >
                ...
                {/* Keeping Trajectory as requested, but maybe opacity 0 initially to avoid 'clutter' until animated? 
            Actually user provided code shows it visible. I'll keep it as is. */}
                <motion.path
