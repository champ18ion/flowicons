...
const ArrowBigDownDash = forwardRef<AnimatedIconHandle, AnimatedIconProps>(
    (
        ...
        // Hover: Nudge Down (Point to dash)
        ...
        // Action: Slam Down + Squash Dash
        const playSlam = useCallback(async () => {
            // 1. Wind up
            await animate(".arrow", { y: -4 }, { duration: 0.2, ease: "easeOut" });

            // 2. Smash
            const smashPromise = animate(".arrow", { y: 3 }, { duration: 0.15, ease: "backIn" });
            const dashSquash = animate(".dash", { scaleX: 1.5, opacity: 0.3, y: 1 }, { duration: 0.15, delay: 0.1 }); // Impact
