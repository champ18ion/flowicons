...
const ArrowBigRight = forwardRef<AnimatedIconHandle, AnimatedIconProps>(
    (
        ...
        const playBump = useCallback(async () => {
            // 1. Wind up (Left)
            await animate(".arrow", { x: -5 }, { duration: 0.2, ease: "easeOut" });

            // 2. Bump Right (past target)
            await animate(".arrow", { x: 6 }, { duration: 0.15, ease: "backIn" });
